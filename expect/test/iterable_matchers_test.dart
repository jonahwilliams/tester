...
testIsIn() {
  ...
  shouldFail(0, isIn('a string'), endsWith('not an <Instance of \'Pattern\'>'));

  // Invalid arg
  // expect(() => isIn(42), throwsArgumentError); TODO
}
...
testEveryElement() {
  ...
  shouldFail('not iterable', everyElement(1),
      endsWith('not an <Instance of \'Iterable<dynamic>\'>'));
}

testNestedEveryElement() {
  var d = [
    ['foo', 'bar'],
    ['foo'],
    []
...
testAnyElement() {
  ...
  shouldFail(e, anyElement(2), 'Expected: some element <2> Actual: [1, 1, 1]');
...
testContainsAll() {
  ...
  shouldFail(
      ...
      'Actual: <1> '
      "Which: not an <Instance of \'Iterable<dynamic>\'>");
  shouldFail(
      [-1, 2],
